...
module pulse_measurement_v3 #(
    ...
    always_comb begin
        ...
        // ST_POST registers the final post-baseline sample into post_sum first.
        // The following cycle (ST_AREA_PREP) derives bpost from that registered
        // final sum.  This deliberately removes the former adc_i -> 48-bit
        // post_sum adder -> bpost -> baseline-pair/DSP-input path from one cycle.
        bpost_calc = post_sum >>> post_shift;
    end
        ...
        if (!rstn_i) begin
            ...
            if (clear_state_i || config_changed || !measurement_enable_i) begin
                ...
                case (state)
                    ...
                    ST_POST: begin
                        if (adc_saturated) begin
                            ...
                            if (post_count + 1'b1 >= post_baseline_samples_i) begin
                                // post_sum <= post_sum + adc_area_ext above captures
                                // the final post-baseline sample at this clock edge.
                                // Do NOT also form bpost/baseline_pair in this cycle:
                                // that was the routed 125 MHz critical path.
                    ...
                    ST_AREA_PREP: begin
                        // One extra 125 MHz pipeline stage (8 ns):
                        // post_sum is already the COMPLETE, REGISTERED post-baseline
                        // sum from the previous ST_POST cycle.  Form bpost and the
                        // baseline pair only from registered values here.
                        //
                        // Numerically this is identical to the old expression:
                        //   (old_post_sum + final_adc_sample) >>> post_shift
                        // because old_post_sum + final_adc_sample was registered
                        // into post_sum at the preceding clock edge.
                        bpost_o <= bpost_calc;
                        baseline_pair_reg <=
