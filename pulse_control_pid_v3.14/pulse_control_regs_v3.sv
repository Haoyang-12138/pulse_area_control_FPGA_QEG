...
// Unified runtime register map for GPIO timing, qualified external reference triggers,
// target-pulse reference selection, PID feedback, OUT2 sequencing, FIFO logging,
// and debug status.
...
module pulse_control_regs_v3 #(
    ...
    parameter int MAX_DAC_SEGMENTS = 32
)(
    input  logic                 clk_i,
    input  logic                 rstn_i,
    ...
    // DIO0_P must remain HIGH for this many 200 MHz samples before it is
    // accepted.  One point is 5 ns; zero is treated as one in the front-end.
    output logic [15:0]          external_trigger_min_high_points_o,

    // Reference/PID target pulse selection, relative to REFERENCE_START.
    // start = 0..31, count = 1..32, and start+count must be <= 32.
    output logic [4:0]           reference_target_start_index_o,
    output logic [5:0]           reference_target_pulse_count_o,
    ...
    output logic [5:0]           dac2_segment_count_shadow_o,
    ...
    input  logic [5:0]           dac2_active_segment_count_i,

    input  logic [31:0]          external_trigger_count_i,
    input  logic [31:0]          external_trigger_active_delay_cycles_i,
    input  logic [15:0]          external_trigger_active_min_high_points_i,
    input  logic                 external_trigger_overrun_i,

    // Reference-selector diagnostics.
    input  logic                 reference_window_active_i,
    input  logic [31:0]          last_reference_id_i,
    input  logic signed [31:0]   last_reference_sum_area_i,
    input  logic [5:0]           last_reference_selected_count_i,
    input  logic                 last_reference_valid_i,
    input  logic                 reference_incomplete_i,
    input  logic                 reference_sum_overflow_i,
    ...
    localparam logic [31:0] VERSION = 32'h50433037; // "PC07"
    ...
    logic reference_incomplete_seen;
    logic reference_sum_overflow_seen;
    ...
    assign status_word = {
        4'd0,
        reference_window_active_i,       // 27
        reference_sum_overflow_seen,     // 26
        reference_incomplete_seen,       // 25
    ...
    always_ff @(posedge clk_i) begin
        if (!rstn_i) begin
            ...
            external_trigger_min_high_points_o    <= 16'd2; // 10 ns @ 200 MHz
            reference_target_start_index_o        <= 5'd0;
            reference_target_pulse_count_o        <= 6'd1;
            ...
            dac2_segment_count_shadow_o <= 6'd4;
            ...
            reference_incomplete_seen       <= 1'b0;
            reference_sum_overflow_seen     <= 1'b0;
            ...
            if (reference_incomplete_i)
                reference_incomplete_seen <= 1'b1;
            if (reference_sum_overflow_i)
                reference_sum_overflow_seen <= 1'b1;
                ...
                if ((sys_addr_i[19:0] >= 20'h00100) &&
                    ...
                    if (sys_addr_i[2] == 1'b0)
                        ...
                        gpio_pulse_end_cycles_shadow_o[sys_addr_i[6:3]] <= sys_wdata_i;
                         ...
                         (sys_addr_i[19:0] <  20'h00300)) begin
                    if (sys_addr_i[2] == 1'b0)
                        dac2_segment_level_q16_shadow_o[sys_addr_i[7:3]] <= sys_wdata_i[15:0];
                    else
                        dac2_segment_duration_cycles_shadow_o[sys_addr_i[7:3]] <= sys_wdata_i;
                end
                else begin
                    case (sys_addr_i[19:0])
                        ...
                        // 8 arm the next complete selected reference-pulse snapshot.
                        20'h00004: begin
                            // The Red Pitaya system bus can hold sys_wen_i HIGH
                            // for more than one clk_i cycle for one MMIO write.
                            // Scalar configuration writes are idempotent, but
                            // COMMAND bits are one-shot actions: POP/APPLY/ARM/
                            // MANUAL-GROUP must execute exactly once per bus
                            // transaction. sys_ack_o is the registered response
                            // to the previous cycle's sys_en, so !sys_ack_o is
                            // true only on the first, not-yet-acknowledged cycle
                            // of this write transaction.
                            if (!sys_ack_o) begin
                        ...
                        20'h00088: external_trigger_min_high_points_o <= sys_wdata_i[15:0];
                        20'h0008C: reference_target_start_index_o <= sys_wdata_i[4:0];
                        20'h00090: reference_target_pulse_count_o <= sys_wdata_i[5:0];

                        20'h00180: dac2_period_cycles_shadow_o <= sys_wdata_i;
                        20'h00184: dac2_segment_count_shadow_o <= sys_wdata_i[5:0];
            ...
            if ((sys_addr_i[19:0] >= 20'h00100) &&
                ...
                if (sys_addr_i[2] == 1'b0)
                    ...
                    sys_rdata_o <= gpio_pulse_end_cycles_shadow_o[sys_addr_i[6:3]];
                     ...
                     (sys_addr_i[19:0] <  20'h00300)) begin
                if (sys_addr_i[2] == 1'b0)
                    sys_rdata_o <= {16'd0, dac2_segment_level_q16_shadow_o[sys_addr_i[7:3]]};
                else
                    sys_rdata_o <= dac2_segment_duration_cycles_shadow_o[sys_addr_i[7:3]];
            end
                ...
                case (sys_addr_i[19:0])
                    ...
                    20'h00088: sys_rdata_o <= {16'd0, external_trigger_min_high_points_o};
                    20'h0008C: sys_rdata_o <= {27'd0, reference_target_start_index_o};
                    20'h00090: sys_rdata_o <= {26'd0, reference_target_pulse_count_o};
                    20'h00180: sys_rdata_o <= dac2_period_cycles_shadow_o;
                    20'h00184: sys_rdata_o <= {26'd0, dac2_segment_count_shadow_o};
                    ...
                    20'h00380: sys_rdata_o <= {26'd0, dac2_active_segment_count_i};
                    ...
                    20'h00390: sys_rdata_o <= {16'd0, external_trigger_active_min_high_points_i};
                    20'h00394: sys_rdata_o <= last_reference_id_i;
                    20'h00398: sys_rdata_o <= last_reference_sum_area_i;
                    20'h0039C: sys_rdata_o <= {
                        22'd0,
                        reference_window_active_i, // bit9
                        last_reference_valid_i,    // bit8
                        2'd0,
                        last_reference_selected_count_i};
                    default: sys_rdata_o <= 32'd0;
                endcase
            end
        end
